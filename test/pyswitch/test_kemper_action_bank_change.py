...
#########################################################################################################
    ...
    def test_messages_keep_rig(self):
        # value 8 = bank 1 (index 1), rig 3 (index 3)
        self._test_messages_keep_rig(up = True,  mapping_value = 8, exp_bank = 2, exp_rig = 3)
        self._test_messages_keep_rig(up = False, mapping_value = 8, exp_bank = 0, exp_rig = 3)

        # Wraparound
        self._test_messages_keep_rig(up = False, mapping_value = 3, exp_bank = NUM_BANKS - 1, exp_rig = 3)
        self._test_messages_keep_rig(up = True,  mapping_value = NUM_BANKS * NUM_RIGS_PER_BANK - 2, exp_bank = 0, exp_rig = 3)

    def _test_messages_keep_rig(self, up, mapping_value, exp_bank, exp_rig):
        if up:
            action = BANK_UP(keep_rig = True)
        else:
            action = BANK_DOWN(keep_rig = True)
        ...
        # Push must preselect the target bank, then select the same rig slot
        self.assertEqual(len(appl.client.set_calls), 2)
        self.assertEqual(appl.client.set_calls[0], {
        ...
        self.assertEqual(appl.client.set_calls[1], {
            "mapping": MAPPING_RIG_SELECT(exp_rig),
            "value": 1
        })
        self.assertEqual(appl.client.set_calls[1]["mapping"].name, MAPPING_RIG_SELECT(exp_rig).name)
        self.assertEqual(appl.shared["morphStateOverride"], 0)

        action.release()

        # Release must send the rig select off (0) value
        self.assertEqual(len(appl.client.set_calls), 3)
        self.assertEqual(appl.client.set_calls[2], {
            "mapping": MAPPING_RIG_SELECT(exp_rig),
            "value": 0
        })
        self.assertEqual(appl.client.set_calls[2]["mapping"].name, MAPPING_RIG_SELECT(exp_rig).name)

    def test_messages_keep_rig_no_value(self):
        for up in (True, False):
            action = BANK_UP(keep_rig = True) if up else BANK_DOWN(keep_rig = True)
            ...
            # Nothing must be sent when the current rig is unknown
